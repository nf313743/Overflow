...
export const { handlers, signIn, signOut, auth } = NextAuth({
    ...
    callbacks: {
        // Called when refreshing page, moving between components, sending authenticated request etc
        async jwt({token, account}) {
            const now = Math.floor(Date.now() / 1000);

            
            // This info is only available on the first sign in
            ...
            // if access token is still valid/not expired then return it
            if (token.accessTokenExpires && now < token.accessTokenExpires) {
                return token;
            }

            // Otherwise, it has expired so get new token
            try {
